...
%
% NOTE: FOR NOW, PARSING DIFFERENT FILES DO NOT APPEND DATA TO EXISTING
% FIELDS. ONLY NEW DATAGRAMS ARE COPIED
%
% INPUT VARIABLES
%
...
% required. As further codes work with the data contained in FABC
% structure, these derived data can be recorded back into the FABC, with
% the 'a' code set to 'X'
%
% b: data dimensions types (rows/columns)
%   P1: ping-like column-vector
%   PB: ping/beam array
%   PT: ping/transmit sector array
%   PS: ping/samples array (note: samples are not sorted, this is not equivalent to range !)
%   D1: datagram-like column-vector (for attitude or position data)
%   DE: datagram/entries-per-datagram array (for attitude or position data)
%
% More codes for the 'b' part will be created if the storage of other
% datagrams needs them. As further codes work with the data contained in FABC
% structure, these derived data can be recorded with other dimensions
% types.They are not listed fully here but they may include:
...
%
% c: data type
%   from SIMRAD datagram variable names for original data
%   from user's imagination for derived data in further code
%
% RESEARCH NOTES
%
% convmat2fabc is the third and last converting function of the series
% "convxtf2all/convall2mat/convmat2fabc".
% - convxtf2all allows turning XTF files into the more widely supported ALL
% format. It also allows reintroducing the parameters of acquisition into
% the file, for complex processing.
% - convall2mat is a general-purpose ALL file parsing function. It is used
% to read our EM3000 data (converted from XTF), to read other sonars data,
% and the original ethernet output of our system (DataDistrib files)
% - convmat2fabc allows to store the data from the unconvenient EM datagram
% format to a more useful structure on which further code will be directed.
%
% convxtf2all typically turns a ~20Mb XTF file into a ~13MB ALL file in
% ~40sec, which convall2mat turns into a -11Mb MAT file in ~40sec, which
% convmat2fabc turns into to a ~11Mb FABC structure in ~8sec. For our EM3000
% data, the idea is to run the XTF to ALL to MAT conversion only once and
% storing ALL and MAT files on the computer, thus reducing the need for
% parsing the original XTF data at each different processing. In effect,
% convmat2fabc runs faster thanks to the convenient MAT format and can then
% be run at the beginning of each different processing code without
% significant time consumption.
%
% NEW FEATURES
%
% - v1.1.1:
%   - fixed watercolumn data parsing for when some datagrams are missing.
%   - height datagram supported
% - v1.1:
        ...
        if ~isfield(FABCdata, 'WC_P1_Date')
            ...
            for iP = 1:nPings
                ...
                for iD = 1:nDatagrams
                    ...
                    FABCdata.WC_PB_TransmitSectorNumber(iP,iBeams) = EM_WaterColumn.TransmitSectorNumber{iDatagrams(iD)};
